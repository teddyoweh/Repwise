...
beforeEach(function () {
    ...
    Cache::forget('github_stars_Repwise_repwise');
});

it('gets stars count from GitHub API', function () {
    // Mock HTTP response
    Http::fake([
        'api.github.com/repos/Repwise/repwise' => Http::response([
            'stargazers_count' => 125,
        ], 200),
    ]);

    // Call the service
    $result = $this->service->getStarsCount('Repwise', 'repwise');
    ...
    Http::assertSent(function ($request) {
        return $request->url() === 'https://api.github.com/repos/Repwise/repwise' &&
...
it('uses cached stars count on subsequent calls', function () {
    ...
    $firstResult = $this->service->getStarsCount('Repwise', 'repwise');
    ...
    $secondResult = $this->service->getStarsCount('Repwise', 'repwise');
...
it('returns 0 when API call fails', function () {
    ...
    Http::fake([
        'api.github.com/repos/Repwise/repwise' => Http::response(null, 500),
    ]);
...
it('formats small numbers as plain numbers', function () {
    ...
    // Call the service
    $result = $this->service->getFormattedStarsCount('Repwise', 'repwise');
